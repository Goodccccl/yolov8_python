...
CLI_HELP_MSG = \
    ...
    YOLOv8 'yolo' CLI commands use the following syntax:
        ...
        Where   TASK (optional) is one of [detect, segment, classify]
                MODE (required) is one of [train2, val, predict2, export]
                ARGS (optional) are any number of custom 'arg=value' pairs like 'imgsz=320' that override defaults.
                    See all ARGS at https://docs.ultralytics.com/cfg or with 'yolo cfg'

    1. Train a detection model for 10 epochs with an initial learning_rate of 0.01
        yolo detect train2 data=coco128.yaml model=yolov8n.pt epochs=10 lr0=0.01

    2. Predict a YouTube video using a pretrained segmentation model at image size 320:
        yolo segment predict2 model=yolov8n-seg.pt source='https://youtu.be/Zgi9g1ksQHc' imgsz=320
...
def entrypoint(debug=''):
    ...
    - specifying the mode, either 'train2', 'val', 'test', or 'predict2'
    ...
    modes = 'train2', 'val', 'predict2', 'export'
    special = {
    ...
    if mode is None:
        mode = DEFAULT_CFG.mode or 'predict2'
    ...
    task = model.task
    overrides['task'] = task
    if mode == 'predict2' and 'source' not in overrides:
        overrides['source'] = DEFAULT_CFG.source or ROOT / "assets" if (ROOT / "assets").exists() \
            else "https://ultralytics.com/images/bus.jpg"
        LOGGER.warning(f"WARNING ⚠️ 'source=' is missing. Using default 'source={overrides['source']}'.")
    elif mode in ('train2', 'val'):
...
if __name__ == '__main__':
    # entrypoint(debug='yolo predict2 model=yolov8n.pt')
    entrypoint(debug='')
