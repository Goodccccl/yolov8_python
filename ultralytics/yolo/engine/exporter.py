...
class Exporter:
    ...
    def _export_onnx(self, prefix=colorstr('ONNX:')):
        ...
        # input_shape = torch.randn((1, 3, 384, 640), device=device)
        torch.onnx.export(
    ...
    def _export_tflite(self, keras_model, int8, data, nms, agnostic_nms, prefix=colorstr('TensorFlow Lite:')):
        ...
        if int8:
            ...
            dataset = LoadImages(check_det_dataset(check_yaml(data))['train2'], imgsz=imgsz, auto=False)
    ...
    def _pipeline_coreml(self, model, prefix=colorstr('CoreML Pipeline:')):
        ...
        else:  # linux and windows can not run model.predict2(), get sizes from pytorch output y
