...
Usage - sources:
    $ yolo task=... mode=predict2  model=s.pt --source 0                         # webcam
                                                img.jpg                         # image
                                                vid.mp4                         # video
                                                screen                          # screenshot
                                                path/                           # directory
...
Usage - formats:
    $ yolo task=... mode=predict2 --weights yolov8n.pt          # PyTorch
...
class BasePredictor:
    ...
    def stream_inference(self, source=None, model=None):
        ...
        # setup source every time predict2 is called
        self.setup_source(source if source is not None else self.args.source)

        # check if save_dir/ labels file exists
        ...
        for batch in self.dataset:
            ...
            with self.dt[0]:
                ...
                #     for h in range(64):
                #         for w in range(64):
                #             with open(r'F:/234.txt', 'a') as f:
                #                 f.write(str(im[c][h][w])+'\n')
                # f.close()
                if len(im.shape) == 3:
